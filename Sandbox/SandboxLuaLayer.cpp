...
namespace lp
{
	...
	SandboxLuaLayer::SandboxLuaLayer() :
		zr::Layer("LuaLayer"),
		mLuaVM()
	{
		if (mLuaVM) {
	...
	void SandboxLuaLayer::onDetach()
	{
		if (mLuaVM)
			mLuaVM.runFunction("onDetach", 0);
	}

	void SandboxLuaLayer::onUpdate(const zr::Time& elapsedTime)
	{
		if (mLuaVM) {
	...
	void SandboxLuaLayer::onEvent(zr::Event& e)
	{
		if (mLuaVM)
			mLuaVM.runFunction("onEvent", 0, e);
	}

	void SandboxLuaLayer::onImGuiRender()
	{
	}

	int SandboxLuaLayer::testFunction(unsigned a, const char* b, double c)
	{
		if (mLuaVM)
	...
	void SandboxLuaLayer::drawEditorWindow()
